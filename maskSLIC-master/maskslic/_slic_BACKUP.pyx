    ...
    with nogil:
        for i in range(max_iter):
            ...
            for k in range(n_segments):
                ...
                # gg = None
                # if i==0:
                #     gg = GeneralizedGamma(image_zyx[z_min:z_max, y_min:y_max, x_min:x_max, :], verbose=True)
                # else:
                ...
                for z in range(z_min, z_max):
                    ...
                    for y in range(y_min, y_max):
                        ...
                        for x in range(x_min, x_max):
                            ...
                            dist_center =(dz + dy + (sx * (cx - x)) ** 2) * spatial_weight # (math.sqrt((cx-x)**2 + (cy-y)**2))/sx#
                            # gg = GeneralizedGamma(image_zyx)
                            #dist_color = gg.likelihood_distance(image_zyx[z, y, x, 0], 0.3, dist_center)
                            # dist_color = image_zyx[z, y, x, 0] - segments[k,0]
                            ...
                            if slic_zero:
                                ...
                                if not only_dist:
                                    # print(dist_center)
                                    if dist_center == 0.0:
                                        dist_center = 0.000000001
                                    total_distance = dist_color + dist_center#(1 - np.exp(1/dist_center))
                                    # dist_center += (dist_color*100)
                            #assign new distance and new label to voxel if closer than other voxels
                            if distance[z, y, x] > total_distance:#dist_center):
                                nearest_segments[z, y, x] = k
                                distance[z, y, x] = total_distance #dist_center
                                #record change
                                change = 1
