...
class GeneralizedGamma():
    ...
    def __init__(self, data, verbose=False, plot=False):
        ...
        c1 = np.sum(np.log(data)) / self.N
        c2 = np.sum(((np.log(data) - c1) ** 2)) / self.N
        c3 = np.sum(((np.log(data) - c1) ** 3)) / self.N
    ...
    def equations(self, vars):
        ...
        sigma, kappa, v = vars
        eq1 = -self.c1 + np.log(sigma) + (special.polygamma(0, kappa) - np.log(kappa))/v  
